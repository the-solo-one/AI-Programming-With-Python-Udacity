...
def process_image(image):
    ...
    # TODO: Processing of a PIL image for use in a PyTorch model
...
def predict(image_path, model, topk=5):
    ...
    # TODO: Implementing the code to predict the class from an image file
    image = process_image(image_path)
    image = torch.from_numpy(image).type(torch.FloatTensor)
    image = image.unsqueeze(0) 
    image = image.to(device)
    
    # for making sure no drop; otherwise, the output will be random
    model.eval()
